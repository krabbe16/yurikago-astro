...
補足として `extends: ['@nuxtjs']` は @nuxtjs/eslint-config のルールを適用するという意味です。パッケージ名の "/eslint-config" という部分は省略して記述する事ができます。

参考: https://github.com/eslint/eslint/blob/v6.8.0/docs/developer-guide/shareable-configs.md#npm-scoped-modules

`plugins: ['@typescript-eslint']` は @typescript-eslint/eslint-plugin のルールを読み込む（利用できるようにする）という意味です。パッケージ名の "/eslint-plugin" という部分は省略して記述する事ができます。
