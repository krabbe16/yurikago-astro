---
title: "マークダウンを校正してPDF形式のスライドに変換する(Slidev+textlint)"
...
> reveal.jsやremarkなどのマークダウン形式でのスライド作成ツールとほぼ同様の記法で、スライドを作成できます。大抵のツールでは、ある程度決まったレイアウトになるのですが、Slidevの場合は拡張性がかなり高いです。Vue Componentや、WindiCSSでスタイリングすることでページごとに独自のスタイルを当てて自由なレイアウトで構築することも出来きます。

…マークダウン形式でスライドを作成できます。苦悶と哀愁に満ちたスライド作成作業に光明が差しました。

参考: https://zenn.dev/ryo_kawamata/articles/introduce-slidev
...
参考: https://sli.dev/guide/install.html#starter-template
...
参考: https://ja.sli.dev/guide/exporting.html

## Slidevの実装

### Markdownシンタックス

Markdownの機能に加えて、インラインHTMLとVueコンポーネントがサポートされています。
...
参考: https://ja.sli.dev/guide/syntax.html
...
参考: https://ja.sli.dev/themes/gallery.html

### 環境変数の参照

いろいろ試している時にたまたま見つけて、需要があるのか微妙ですが書いておきます。

Slidevは内部でViteを使っています。なので `.env` に定義した環境変数をVue Componentの `import.meta.env.xxx` で参照できます。
...
参考: https://ics.media/entry/220404/
...
| sudachi-synonyms-dictionary            | Sudachi同義語辞書を扱うパッケージ。textlint-rule-no-synonymsで必要。 |
...
参考: https://textlint.github.io/docs/getting-started.html
...
参考: https://dev.classmethod.jp/articles/markdown-writing-with-textlint-ci/
...
![](../../assets/images/post/20f0db0e9126-20220611.png)
