...
まず既存の画像を WebP 形式に変換します。Google Chrome Labs が開発した 画像圧縮アプリ を利用しました。サンプルとして以下の PNG 画像をロスレス方式で WebP 画像に変換しました。
...
出力内容を変更する方法は ドキュメント に記述されています。ドキュメントを参考に vue ファイルのロジックを修正しました。
...
`renderer.image` のロジックは GitHub の ソースコード を参照しました。
